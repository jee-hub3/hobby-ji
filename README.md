...
1. Download from [Releases](../../releases) — two `.exe` files are attached:
   `Claude-Usage-Widget-Setup-*.exe` (installer) or `Claude-Usage-Widget-*.exe`
   **without** "Setup" in the name (portable, no install — just run it)
...
1. [Releases](../../releases)에서 `.exe` 파일 2개 중 하나 다운로드 —
   `Claude-Usage-Widget-Setup-*.exe`(설치형) 또는 "Setup"이 **안 붙은**
   `Claude-Usage-Widget-*.exe`(portable, 설치 없이 바로 실행)
